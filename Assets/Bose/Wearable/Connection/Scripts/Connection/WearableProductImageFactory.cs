...
namespace Bose.Wearable
{
	internal sealed class WearableProductImageFactory : ScriptableObject
	{
		...
		private const string DuplicateProductMappingWarning =
			"[BoseWearable] Product Image Mapping for ProductType [{0}] will not be added as it already" +
			" exists (duplicate).";

		private const string DuplicateVariantMappingWarning =
		...
		private void Setup()
		{
			...
			for (var i = 0; i < _productMappings.Length; i++)
			{
				...
				{
					Debug.LogWarningFormat(DuplicateProductMappingWarning, _productMappings[i].productType);
				}
			}
			...
			for (var i = 0; i < _variantMappings.Length; i++)
			{
				...
				{
					Debug.LogWarningFormat(DuplicateVariantMappingWarning, _variantMappings[i].variantType);
				}
			}
		}
