	...
	internal sealed class TimingDebugUIWindow : MonoBehaviour
	{
		...
		private void Update()
		{
			...
			_frameTimestamp.text = _dataMetrics.LastTimestamp.HasValue
				? string.Format(DebuggingConstants.SecondsFormat, _dataMetrics.LastTimestamp)
				: string.Empty;

			_frameDelta.text = _dataMetrics.LastDeltaTime.HasValue
				? string.Format(DebuggingConstants.SecondsFormat, _dataMetrics.LastDeltaTime)
				: string.Empty;

			_renderFramesPerSecond.text = string.Format(
				DebuggingConstants.FramesPerSecondFormat,
			...
			_lastTransferInterval.text = _dataMetrics.LastTransferInterval.HasValue
				? string.Format(DebuggingConstants.SecondsFormat, _dataMetrics.LastTransferInterval)
				: string.Empty;
			
			_lastTransferAge.text = _dataMetrics.LastTransferAge.HasValue
				? string.Format(DebuggingConstants.SecondsFormat, _dataMetrics.LastTransferAge)
				: string.Empty;
			...
			_unityToDeviceOffset.text = _dataMetrics.UnityToDeviceOffset.HasValue
				? string.Format(DebuggingConstants.SecondsFormat, _dataMetrics.UnityToDeviceOffset)
				: string.Empty;
		}
		...
		private string GetRenderFrameCapturesString()
		{
			...
			for (var i = 0; i < _capturedSensorFramesPerRenderFrame.Length; i++)
			{
				...
				{
					_captureStringBuilder.Append(EmptySpace);
				}
			}
